...
try:
    with open(
        os.path.join(os.path.dirname(__file__), "../prompt/supervisor.md"),
        "r",
        encoding="utf-8",
    ) as f:
        system_prompt = f.read()
except FileNotFoundError:
    raise FileNotFoundError("supervisor.md not found")


def supervisor_prompt(state: State):

    prompt: PromptType = [PromptType(role=ChatRole.SYSTEM, content=system_prompt)]
